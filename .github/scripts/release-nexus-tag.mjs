...
const DOCKER_STAGING_REGISTRY =
    process.env.DOCKER_STAGING_REGISTRY || 'uniportcr.artifacts.inventage.com';
const DOCKER_RELEASE_REGISTRY =
    process.env.DOCKER_RELEASE_REGISTRY || 'docker-registry.inventage.com:10093';

if (!NEXUS_USER || !NEXUS_PW) {
    console.error(
        `${chalk.bold('NEXUS_USER')} and ${chalk.bold('NEXUS_PW')} have to be set.`
    );
    process.exit(1);
}
...
try {
    ...
    const componentsResponse = await fetch(nexusRequestUrl, {
        headers: {
            Authorization: `Basic ${Buffer.from(`${NEXUS_USER}:${NEXUS_PW}`).toString(
                'base64'
            )}`,
        },
    });
    ...
    if (!componentsResponse.ok) {
        console.error(
            `HTTP request to ${nexusRequestUrl} failed: ${componentsResponse.status} ${componentsResponse.statusText}`
        );
        process.exit(1);
    }
...
if (components.length < 1) {
    console.error(
        `No docker components found associated with tag ${chalk.bold(NEXUS_TAG)}`
    );
    process.exit(1);
}
...
components.forEach(async (c) => {
    ...
    console.info(
        `Moving docker image ${chalk.bold(source)} to ${chalk.bold(dest)}…`
    );
